 ...
 * import
 * org.springframework.statemachine.support.StateMachineInterceptorAdapter;
 ...
 * import
 * com.subsede.amc.catalog.model.quote.QuotationSpringSMConfig.QuoteStates;
 ...
 * assertEquals(today.get(Calendar.DAY_OF_MONTH),
 * validUpto.get(Calendar.DAY_OF_MONTH));
 * }
 * 
 * @Test
 ...
 * UserData data = new UserData(ServiceTestHelper.createPackageWithDiscount(),
 * Rating.ONE, 2, 1000.0);
 * try {
 ...
 * State<Quotation> funnyThing = fsm.onEvent(quote,
 * Event.APPROVE_QUOTE.toString(), "");
 ...
 * StateMachine<QuoteStates, Events> sm =
 * QuotationSpringSMConfig.createDefaultMachine();
 * sm.getStateMachineAccessor()
 * .doWithAllRegions(
 * func -> func.addStateMachineInterceptor(new
 * StateMachineInterceptorAdapter<QuoteStates, Events>() {
 * 
 * @Override
 * public Exception stateMachineError(StateMachine<QuoteStates, Events> sm,
 * Exception ex) {
 ...
 * logger.
 * info("whether the transition occured ******************* {} current state is {}"
 * , sm.hasStateMachineError(),
 * sm.getState().getId());
 * 
 ...
 * StateMachine<QuoteStates, Events> sm =
 * configurer.createStateMachine(currentState);
 * sm.start();
 ...
 * try {
 * logger.info("Current thread count &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&& {}"
 * , i);
 * StateMachine<QuoteStates, Events> sm =
 * QuotationSpringSMConfig.createDefaultMachine();
 * 
 * StateMachine<QuoteStates, Events> currSM =
 * QuotationSpringSMConfig.resteToState(sm,
 ...
 * }
 * fsm.setPersister(new MemoryPersisterImpl<Quotation>(states,
 * States.INIT.get(), "state"));
 * logger.debug("{}", fsm);
 * return fsm;
 * }
 * 
 ...
 * 
 * @Override
 * public void execute(Quotation paramT, String paramString, Object...
 * paramArrayOfObject) throws RetryException {
